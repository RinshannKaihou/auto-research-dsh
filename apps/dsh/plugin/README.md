...
```bash
dsh plugin --profile web add ./auto-research-v5-0.6.11.tgz --offline
dsh web
```
...
本版合入成果展示和历史专家会话导航修复。专家会话通过持久化父地址打开，已结束专家的 Research 可只读浏览所属项目，并可返回父会话或研究主会话。schema 保持 9，无新增迁移。

`research_publish` 保留必填 `summary` 及所有旧调用方式，可在同一次发布中添加 `display`。展示摘要由当前研究 agent 按研究目标语言填写，不调用额外模型；报告、公式、引用和完整发布说明保留原文。

```json
{
  "title": "精度诱发延迟失稳：文献调研",
  "overview": "整理候选机制、分析方法和下一阶段待验证假设。",
  "sections": [
    {"heading": "已有结果", "items": ["完成文献与公式核对，报告记录证据来源。"]},
    {"heading": "待验证假设", "items": ["候选机制仍需对照实验检验。"]},
    {"heading": "限制", "items": ["文献结论不能直接外推至当前实验环境。"]}
  ],
  "primary_item_id": "report"
}
```

上例是 `display` 的值；`primary_item_id` 必须对应本次 `items` 中真实的文件附件。标题最多 80 字符，摘要最多 400 字符，最多 4 组、每组最多 4 条、每条最多 240 字符。字段可完全省略；填写时校验其结构和附件引用。

展示信息以 `publication-display/v1` 内容保存到本次成果的专用 item `__research_display`，不增加数据库表，不修改历史记录。普通附件列表隐藏该 item；损坏或未知版本的展示信息退回历史样式。`research.presentation.json` 仍只提供项目概览。

成果卡片使用短标题、普通字重的四行预览、可展开的完整 Markdown 和原文入口。正文中的超长标题按段落显示；来源和文件路径可折叠。优先打开明确指定的报告，没有指定时只在存在唯一 Markdown 附件时提供直接入口，其他情况显示材料列表。交付完成不代表科学验证。

前端文件更新后刷新即可改善旧服务中的历史成果。新的发布参数、分块读取 RPC 和角色提示在下一次正常服务加载及新建研究会话时生效。旧服务不能返回超长说明全文时会明确提示，绝不把截断预览冒充全文。无需为应用本补丁强制重启正在运行的研究。

### 项目会话分组

“运行与维护”的项目会话按持久化父会话收纳专家，节点默认折叠；展开节点后显示专家任务编号、任务标题和状态。节点收起时仍显示已加载的专家数量、运行及待处理计数。多个执行会话属于同一节点时，优先依据实际父会话区分；缺少父地址时只在节点核心唯一的情况下使用该节点作为展示上下文，不伪造导航地址。无法确定归属的记录放入单独折叠组。“节点内部协作”的任务清单也默认折叠。

新服务的会话分页直接附带有界专家标题与归属信息；已运行的旧服务在展开节点时按需读取任务标题并缓存，无需调用模型或改写账本。会话总数超过当前页时继续使用“加载更多”，分组数量和专家计数仅统计已加载记录。

### 节点讨论与会话身份

“研究过程”的节点提供“讨论进展”，节点详情提供“围绕此节点讨论”。默认续用该节点已有的独立讨论；“更多 → 新建另一场讨论”显式创建新讨论。讨论使用自己的工作目录并登记到原生会话列表，默认名称包含节点与会话短编号，保留用户自定义名称。

页头和输入区显示当前节点及角色，区分研究主会话、节点执行、讨论和专家；讨论可返回主会话，专家可返回父会话。对话仍使用原生 Chat 界面，身份随实际会话切换。讨论背景固定于创建时，复用不会自动刷新全部背景，可显式查询新记录。打开讨论不会自动发送消息或启动模型，也不会向研究主会话注入指令。
